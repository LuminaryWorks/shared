...
- **Logto** / 任意 OIDC IdP（JWKS + Discovery�?- **external_oidc** �?企业 Azure AD、Okta、蓝�?OIDC 直连
...
各产�?`package.json` 使用 `"@luminaryworks/auth-core": "file:../packages/luminary-auth-core"`（路径按仓库相对位置调整）�?
**注意**：`@nestjs/common` / `@nestjs/core` �?peerDependency，由宿主 NestJS 项目提供，避免重复安装导致类型冲突�?
```typescript
